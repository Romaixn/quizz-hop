...
final class ConsoleProjectionFactory implements MessageSubscriberInterface
{
    ...
    {
        if ($message->getPayload() instanceof QuizzWasCreated) {
            $this->onQuizzCreated($message->getPayload());
        }
    }
    ...
    public static function getHandledMessages(): iterable
    {
        yield DomainMessage::class => [
            'from_transport' => 'quizz',
            'bus' => 'messenger.bus.event.async',
        ];
    }

    private function onQuizzCreated(QuizzWasCreated $event): void
    {
        $this->logger->info('A new quizz was created ' . $event->name->toString());
    }
}
